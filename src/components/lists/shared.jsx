...
export function EventRow({ event, course, onToggle, onEdit }) {
  ...
  const P = PRIORITY[event.priority] || PRIORITY.medium;
  const col = event.color || course?.color || E.dot;
  ...
  return (
    <div className={`group flex items-center gap-3 rounded-xl border px-3 py-3 hover:bg-accent/30 transition ${event.flag ? 'border-rose-400 bg-rose-50' : overdue ? 'border-rose-300 bg-rose-50' : 'border-border/60'} ${done ? 'opacity-60' : ''}`}>
      <span className="w-1.5 self-stretch rounded-full shrink-0" style={{ backgroundColor: col }} />
